...
import { useInvitations } from "@/hooks/useInvitations"; // Adjust path if needed
...
const InvitationCenter = () => {
  // Utilizing your custom hook for auto-polling and state management
  const { invitations, setInvitations, refresh } = useInvitations();
  ...
  const handleAction = async (id: string, action: "accept" | "reject") => {
    ...
    try {
      ...
      // 2. Rollback/Resync if server call fails
                    ...
                    {invitations.length === 0 ? (
                      ...
                      invitations.map((invite: any) => (
                        <motion.div
                          layout
                          key={invite._id}
                          className="p-4 bg-slate-950/50 border border-slate-900 rounded-2xl flex flex-col gap-4 hover:border-slate-700 transition-colors"
                        >
